...
- responsible: ["ФИО или подразделение"]
Поручение — это задача, которая:
- содержит явные слова: "поручение", "обязать", "назначить", "контроль", "исполнение", "отчёт", "срок", "ответственный";
- относится к служебной деятельности (не бытовые задачи);
- имеет указание на ответственного (ФИО или должность) и/или срок исполнения.
    ...
    "assignments": [
        {
            ...
            "responsible": ["ФИО или подразделение"],
            "source_fragment": "Фрагмент текста из документа",
            "confidence": 0.9
        }
    ]
}
...
Правила:
1. Если нет явного указания на поручение и ответственного — НЕ выдумывай.
2. Если дедлайн не указан — ставь null.
3. Если ответственный не указан — ставь ["Не указан"], но только если поручение явное.
4. Для бытовых задач (например, "переставить шкафы") — не возвращай.
